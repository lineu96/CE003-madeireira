...
## Soma das atividades remotas (soma máxima atual: 90).

<center>
```{r, echo=FALSE}
pander::pander(posicao(notas$SOMA_REMOTAS))
```
</center>
...
p1 = ggplot(notas) + 
  geom_histogram(aes(x=SOMA_REMOTAS,
                     y=..density..), 
                 position="identity",
                 col = 1,
                 lwd = 1,
                 breaks = hist(notas$SOMA_REMOTAS,
                               plot = F)$breaks) + 
  geom_density(aes(x=SOMA_REMOTAS,
                   y=..density..),
               col = 4,
               lwd = 1) + 
  ylab("Densidade") +
  xlab("") +
  ggtitle("Soma atividades remotas")+
  theme_classic() + theme(legend.position = 'none',
                          plot.title = element_text(family = "Helvetica", 
                                                    face = "bold", 
                                                    size = (20),
...
p2 = ggplot(data = notas, 
            mapping = aes(y=SOMA_REMOTAS, x='1')) +
  stat_boxplot(geom ='errorbar')+
  geom_boxplot(alpha = 1)+
  #xlim(c(0,10)) + 
  stat_summary(fun.y=mean, 
